...
function version_check() {
    var a = browser.runtime.getManifest(),
        b = a.version;
...
function handleExactBlock(block_text, tabObj, params) {
    if (block_text.substring(0, 7) === 'exact__') {
        ...
        */
        ...
        if (pos == -1) return;
        else if (tabObj.url.substring(pos + block_url.length).length > 3)
            return;
...
async function checkUrl(tabObj) {
    ...
    if (tabObj !== null) {
        ...
        var permanent_block_list = JSON.parse(
            (await getProperty('appendblocklist')) ?? '[]'
        );
...
async function block(tabObj, params) {
    ...
    if (isPermanent || (await propertyEquals('activated', true))) {
        if (await propertyEquals('wipePage', true)) {
            ...
            await browser.tabs.update(tabObj.id, {
                url: 'http://google.com',
            });
        } else {
            await browser.tabs.update(tabObj.id, {
...
local_storage_check(),
    ...
    browser.tabs.onUpdated.addListener(function (tabID, changeInfo, tabObj) {
        if (
            tabsLastHandled[tabID] &&
            new Date() - tabsLastHandled[tabID] < 500
        ) {
            // Tab was handled in the last half second, do nothing
            return;
        }

        checkUrl(tabObj);
        tabsLastHandled[tabID] = new Date();
    }),
