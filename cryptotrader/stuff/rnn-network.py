...
from database import next_batch_btc, connect_to_database, exit_database, get_num_candles_in_table
...
def train_neural_network(x):
    ...
    with tf.Session() as sess:
        ...
        num_available_candles = get_num_candles_in_table(c, 'BTC_USD')
        batch_counter = 0
        ...
        for epoch in range(hm_epochs):
            ...
            for _ in range(int(num_available_candles/batch_size)):
                #epoch_x, epoch_y = mnist.train.next_batch(batch_size)

                # Pull 780 candles (size of one data point) times batch_size.
                epoch_x, epoch_y = next_batch_btc(c, batch_size, batch_counter)
                batch_counter += 1

                print("{}/{} done".format(batch_counter, num_available_candles / batch_size))
                ...
                _, my_loss = sess.run([optimizer, cost], feed_dict={x: epoch_x, y: epoch_y})
                # cost is the loss for each batch - we want to add them up to
        ...
        test_input, test_output = next_batch_btc(c, batch_size, batch_counter + 5)
